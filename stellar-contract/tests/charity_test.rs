...
fn test_donate_to_charity() {
    ...
    // Event should contain donation information (tuple: address, topics vec, data)
    assert!(last_event.1.len() > 0);
}
...
fn test_donate_to_charity_event_emission() {
    ...
    // Verify event structure (address, topics vec, data)
    assert!(last_event.1.len() >= 2);
}
